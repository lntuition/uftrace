...
static void load_module_symtab(struct symtabs *symtabs, struct uftrace_module *m)
{
...
void load_module_symtabs(struct symtabs *symtabs)
{
	...
	for_each_map(symtabs, maps) {
		...
		if (flags & SYMTAB_FL_USE_SYMFILE) {
			...
			if (access(symfile, F_OK) == 0)
				load_module_symbol_file(&maps->symtab, symfile, 0);
		 ...
		 * support more sophisticated symbol handling.
		 */
		load_symtab(&maps->symtab, maps->libname, 0, flags);
		load_dynsymtab(&dsymtab, maps->libname, 0, flags);
...
struct uftrace_mmap * find_symbol_map(struct symtabs *symtabs, char *name)
{
	...
	for_each_map(symtabs, maps) {
		struct sym *sym = find_symname(&maps->symtab, name);
